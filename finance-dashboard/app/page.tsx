    ...
    <main className="max-w-7xl mx-auto p-6 lg:p-10">
      ...
      {!dataset ? (
        <div className="mt-8 space-y-6">
          <ExcelUploader onLoaded={setDataset} />
          <FeatureGrid />
        </div>
      ) : (
    ...
    <header className="flex items-start justify-between gap-4 flex-wrap">
      <div>
        <div className="inline-flex items-center gap-2 pill bg-brand-50 text-brand-700">
          <Sparkles className="w-3.5 h-3.5" /> Reportes financieros automatizados
        </div>
        <h1 className="mt-3 text-3xl lg:text-4xl font-extrabold text-ink-900 tracking-tight">
          Finance Dashboard
        </h1>
        <p className="mt-2 text-ink-700 max-w-2xl">
          Sube tu Excel, mapea las columnas y obtén variación
          presupuestaria, análisis vertical/horizontal, ratios financieros y
          flujo de caja — todo procesado localmente, exportable a Excel, PDF y
          PNG.
        </p>
      </div>
    </header>
  ...
  const items = [
    {
      icon: BarChart3,
      title: "Variación Presupuesto vs Real",
      desc: "Detecta desviaciones materiales con gráfico de Top variaciones y detalle por cuenta.",
    },
    {
      icon: PieChart,
      title: "Análisis Vertical y Horizontal",
    ...
    },
    {
      icon: LineChart,
      title: "Flujo de Caja y Tendencias",
      desc: "Ingresos vs egresos por periodo con saldo acumulado y proyección visual.",
    },
  ];
  return (
